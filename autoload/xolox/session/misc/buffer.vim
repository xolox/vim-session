...
function! xolox#session#misc#buffer#is_empty() " {{{1
...
function! xolox#session#misc#buffer#prepare(...) " {{{1
  ...
  for bufnr in tabpagebuflist()
    if xolox#session#misc#path#equals(options['path'], bufname(bufnr))
  ...
  if !(found || xolox#session#misc#buffer#is_empty())
    vsplit
  endif
  ...
  call xolox#session#misc#buffer#unlock()
  silent %delete
endfunction

function! xolox#session#misc#buffer#lock() " {{{1
  " Lock a special buffer so that its contents can no longer be edited.
  setlocal readonly nomodifiable nomodified
endfunction

function! xolox#session#misc#buffer#unlock() " {{{1
