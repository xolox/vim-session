...
function! xolox#session#misc#path#which(...) " {{{1
  ...
  " example:
  "
  "     :echo xolox#session#misc#path#which('gvim', 'vim')
  "     ['/usr/local/bin/gvim',
  "      '/usr/bin/gvim',
  "      '/usr/local/bin/vim',
  "      '/usr/bin/vim']
...
function! xolox#session#misc#path#split(path) " {{{1
  ...
  " use the `xolox#session#misc#path#join()` function.
  if type(a:path) == type('')
    if s:windows_compatible
      if a:path =~ '^[\/][\/]'
        " UNC pathname.
...
function! xolox#session#misc#path#join(parts) " {{{1
  ...
  " `xolox#session#misc#path#split()` function and it expects a list of pathname
  " components as returned by `xolox#session#misc#path#split()`.
  if type(a:parts) == type([])
    if s:windows_compatible
      return join(a:parts, xolox#session#misc#path#directory_separator())
    elseif a:parts[0] == '/'
...
function! xolox#session#misc#path#directory_separator() " {{{1
  " Find the preferred directory separator for the platform and settings.
  return exists('+shellslash') && &shellslash ? '/' : '\'
endfunction

function! xolox#session#misc#path#absolute(path) " {{{1
  ...
  if type(a:path) == type('')
    ...
    elseif xolox#session#misc#path#is_relative(path)
    ...
    let parts = xolox#session#misc#path#split(path)
    if s:windows_compatible && parts[0] =~ '^[\/][\/]'
      ...
      let parts[0] = repeat(xolox#session#misc#path#directory_separator(), 2) . parts[0][2:]
    endif
    return xolox#session#misc#path#join(parts)
  endif
  return ''
endfunction

function! xolox#session#misc#path#relative(path, base) " {{{1
  " Make an absolute pathname (the first argument) relative to a directory
  " (the second argument).
  let path = xolox#session#misc#path#split(a:path)
  let base = xolox#session#misc#path#split(a:base)
  while path != [] && base != [] && path[0] == base[0]
    call remove(path, 0)
    call remove(base, 0)
  endwhile
  let distance = repeat(['..'], len(base))
  return xolox#session#misc#path#join(distance + path)
endfunction


function! xolox#session#misc#path#merge(parent, child, ...) " {{{1
  " Join a directory pathname and filename into a single pathname.
  if type(a:parent) == type('') && type(a:child) == type('')
    " TODO Use xolox#session#misc#path#is_relative()?
    if s:windows_compatible
...
function! xolox#session#misc#path#commonprefix(paths) " {{{1
  " Find the common prefix of path components in a list of pathnames.
  let common = xolox#session#misc#path#split(a:paths[0])
  for path in a:paths
    let index = 0
    for segment in xolox#session#misc#path#split(path)
      if len(common) <= index
        break
  ...
  return xolox#session#misc#path#join(common)
endfunction

function! xolox#session#misc#path#encode(path) " {{{1
...
function! xolox#session#misc#path#decode(encoded_path) " {{{1
  " Decode a pathname previously encoded with `xolox#session#misc#path#encode()`.
  return substitute(a:encoded_path, '%\(\x\x\?\)', '\=nr2char("0x" . submatch(1))', 'g')
endfunction

" xolox#session#misc#path#equals(a, b) - Check whether two pathnames point to the same file. {{{1

if s:windows_compatible
  function! xolox#session#misc#path#equals(a, b)
    return a:a ==? a:b || xolox#session#misc#path#absolute(a:a) ==? xolox#session#misc#path#absolute(a:b)
  endfunction
else
  function! xolox#session#misc#path#equals(a, b)
    return a:a ==# a:b || xolox#session#misc#path#absolute(a:a) ==# xolox#session#misc#path#absolute(a:b)
...
function! xolox#session#misc#path#is_relative(path) " {{{1
...
function! xolox#session#misc#path#tempdir() " {{{1
  ...
  if !exists('template')
    throw "xolox#session#misc#path#tempdir() hasn't been implemented on your platform!"
  endif
  while 1
